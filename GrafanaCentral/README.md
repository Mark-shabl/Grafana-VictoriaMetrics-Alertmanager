...
URL для remote write: `http://ВАШ_ДОМЕН_CENTRAL:8428/api/v1/write` (в `GrafanaRemote/.env` указывают только базу без `/api/v1/write`).
...
|-----|------------|--------|
| **GrafanaCentral/.env** | `CENTRAL_HOST` | `monitoring.lan` |
| **GrafanaRemote/.env** | `CENTRAL_URL` | `http://monitoring.lan:8428` |
| **GrafanaCentral/.env** | `MIKROTIK_SNMP_TARGET`, `MIKROTIK_API_HOST` | `mikrotik.lan` |
...
`vmagent`, `node_exporter` и cAdvisor в central используют host network, поэтому локальные метрики собираются через `127.0.0.1` без отдельного запуска `GrafanaRemote/`.
...
Если будет **Permission denied** при записи в `GrafanaCentral/dashboards/`, каталог принадлежит **root** (часто после `sudo git clone/pull`): выполните `sudo bash scripts/download-dashboards.sh` или один раз **`sudo chown -R "$(whoami)":"$(whoami)" dashboards/`**.
...
2. В `GrafanaCentral/.env`: `MIKROTIK_SNMP_COMMUNITY=<вашcommunity>`, **`MIKROTIK_SNMP_TARGET=mikrotik.lan`** (ваш домен роутера).
...
Полный перечень см. **`GrafanaCentral/.env.example`**.
...
Дашборд: Grafana **ID 13679** (после **`scripts/download-dashboards.*`** файл **`GrafanaCentral/dashboards/mktxp.json`**).
...
**Не используете MKTXP:** удалите сервис `mktxp` из `docker-compose.yml`, job `mktxp` из `vmagent/prometheus.yml.template` и уберите строку **`mktxp`** из **`depends_on`** у `vmagent`.

### Если MikroTik нет вообще

**Без роутера MikroTik:** удалите блоки `job_name: snmp_mikrotik` и `job_name: snmp_if_mib`, job `mktxp` из `vmagent/prometheus.yml.template` и строку **`mktxp`** из `depends_on` у `vmagent`; из `docker-compose.yml` удалите сервисы `snmp_exporter` и **`mktxp`**, чтобы не было ошибок scrape.
...
- VictoriaMetrics `unsupported path requested` для `"/api/v1/write/api/v1/write"` — путь `/api/v1/write` указали дважды. У **GrafanaRemote** в `.env` в `CENTRAL_URL` должно быть только `http://хост:8428`. У **vmalert** `-remoteWrite.url` без суффикса `/api/v1/write`; после правки на central: `docker compose up -d`.
- VictoriaMetrics `ignoring series with … labels` для `container_*` и `maxLabelsPerTimeseries` — у метрики слишком много лейблов (часто тяжёлый Docker-образ как NPM MySQL + cAdvisor). В `GrafanaCentral/docker-compose.yml` задано `-maxLabelsPerTimeseries=64`; при необходимости поднимите осторожно или урежьте лейблы через relabel на vmagent.
